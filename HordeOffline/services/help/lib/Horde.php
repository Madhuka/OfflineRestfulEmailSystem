...
HTML;
    ...
    function stylesheetLink($apps = null, $theme = '', $inherit = true)
    {
		...
		$html .= '<script type="text/javascript" src="tree.js"></script> ' . "\n";
        return $html;
    }
    ...
    function getStylesheets($apps = null, $theme = '', $inherit = true)
    {
		...
		$brwsrt=$_SERVER['HTTP_USER_AGENT'];
        $browser_css = array();
        if (stristr($brwsrt,"MSIE")) {
            $browser_css[] = 'ie7.css';
        }
        if (stristr($brwsrt,"opera")) {
            $browser_css[] = 'opera.css';
        }
        if (stristr($brwsrt,"Firefox")&&stristr($brwsrt,"5.0")) {
            $browser_css[] = 'moz14.css';
        }
        if (stristr($brwsrt,"Safari") ){
        ...
        foreach ($apps as $app) {
			...
			$css[] = array('u' => $themes_uri . '/bluewhite/screen.css', 'f' => $themes_fs . '/bluewhite/screen.css');
...
class Horde_Script_Files {
    ...
    function _add($file, $app, $direct, $full = false)
    {
        ...
        $out = $this->_files[$app][] = array('f' => 'horde.js', 'd' =>'1' , 'u' => 'horde.js', 'p' => 'horde.js');
        return $out;
    }
